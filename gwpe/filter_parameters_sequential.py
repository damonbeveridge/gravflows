...
def generate_filter_parameters(
    ...
    with tqdm(desc=progress_desc, total=n_samples, miniters=1, postfix={'saved': saved}, disable=not verbose) as progress:
        for key, sample in enumerate(chirp_masses):
            filter_sampling_job = sample_filter_params(
                chirp_masses=sample,
                cm_range=cm_range,
            ...
            # filters = filter_sampling_job
            progress.update(1)
            # progress.refresh()

            filters_memmap[key, :, :] = np.stack(filter_sampling_job)
